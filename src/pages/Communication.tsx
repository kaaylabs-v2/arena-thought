...
import { Send, Circle, Mail, Bell, X } from "lucide-react";
              ...
              >
                <div className="flex items-center justify-between mb-0.5">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-[11px] text-muted-foreground/60">{getListTime(lastMsg.timestamp)}</span>
          ...
          <div className="space-y-3">
            {sorted.map((ann) => (
              <div
                key={ann.id}
                className={cn(
                  "bg-card border border-border rounded-xl px-5 py-4 border-l-4 transition-opacity",
                  ann.dismissed ? "border-l-transparent opacity-80" : "border-l-accent"
                )}
              >
